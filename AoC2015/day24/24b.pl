...
# part b: same observation, but uneven and 5 presents this time

my ($optimalCount, $optimalQE) = (5, 999999999999); #current best solution
...
sub weasel {
  ...
  if($sum > $total){return;}
  if($sum == $total){
    if($values[0] * $values[1] * $values[2] * $values[3] * $values[4] < $optimalQE){
      $optimalQE = $values[0] * $values[1] * $values[2] * $values[3] * $values[4];
    }
  }
